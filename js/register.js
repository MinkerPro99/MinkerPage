...
async function parseResponseJsonSafe(response) {
  ...
  if (!rawText) return { _rawText: '' };

  try {
const parsed = JSON.parse(rawText);
if (parsed && typeof parsed === 'object') parsed._rawText = rawText;
return parsed;
  } catch {
return { error: `Unexpected non-JSON response (HTTP ${response.status})`, _rawText: rawText };
  }
}
